...
export class AccountsService {
  ...
  async findUserAccounts_withBalance(userId: number) {
    try {
...
WHERE a.customer_id = $1;`,
      ...
      if (!resultArr.rows.length) {
        throw new NotFoundException('Accounts and Balance not found');
      }
  ...
  //this method is used when you create new user -> automatically creates account for him
  async create(
  ...
  //this method is used when you create new saving account -> automatically creates account for him -> automatically create a transaction with 0 balance for this account
  async createForSavingAccount(
    createAccountDto: CreateAccountDto,
    client?: PoolClient,
  ): Promise<AccountDto> {
    const executor = client ?? this.pool;
    ...
    try {
      const newAccount = await this.create(createAccountDto, client);
      ...
      const newTransaction = {
        ...
        money_amount: 0,
        balance: 0,
        details: 'You have successfully created a new savings account',
      };

      await this.transactionsService.create(newTransaction, client);

      const newMessage: CreateMessageDto = {
        customer_id: newAccount.customer_id,
        msg_subject: 'New Savings Account Created',
        msg_status: 'Open',
        msg_body: `Dear Customer, Your new savings account with account number ${newAccount.account_nr} has been successfully created.`,
      ...
      if (!createdMessage) {
        throw new BadRequestException(
          'Failed to create message for the new Account',
        );
      }
